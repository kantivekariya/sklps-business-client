...
import { Check, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Eye, Loader2, Search, Trash2, X } from "lucide-react";
...
export function BusinessManagement() {
    ...
    const fetchBusinesses = async () => {
      ...
      try {
        if (currentTab === "pending") {
          const { data } = await api.get<Business[]>("/admin/pending");
          setBusinesses(data);
  ...
  const handleApprove = async (id: string) => {
    try {
      await api.put(API_ENDPOINTS.BUSINESS_APPROVE(id));
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      await api.delete(API_ENDPOINTS.BUSINESS_DELETE(id));
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  {selectedBusiness.mobile} <br /> {selectedBusiness.email}
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground mb-1">Native Place</span>
                  {selectedBusiness.nativePlace || "N/A"}
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground mb-1">Registration No.</span>
                  {selectedBusiness.registrationNumber || "N/A"}
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground mb-1">Reference Name</span>
                  {selectedBusiness.referenceName || "N/A"}
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground mb-1">Reference Contact</span>
