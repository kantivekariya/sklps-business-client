...
export function JobDetails({ job }: JobDetailsProps) {
  ...
  const onSubmitApplication = async (data: Record<string, string>) => {
    setApplying(true);
    try {
      await api.post(API_ENDPOINTS.JOB_APPLY(job._id), data);
      toast({
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
              <div className="flex-1">
                <div className="flex flex-wrap gap-2 mb-3">
                  <Badge variant="secondary" className="border-primary-foreground/30 bg-primary-foreground/20 text-primary-foreground">
                    {job.category}
                  </Badge>
                  <Badge variant="secondary" className="border-primary-foreground/30 bg-primary-foreground/20 text-primary-foreground">
                    {job.jobType}
            ...
            {job.salaryRange && (
              <div className="mb-6 flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-500/10 p-4 dark:border-emerald-800">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-emerald-700 dark:text-emerald-400">{job.salaryRange}</p>
                </div>
              </div>
            )}
