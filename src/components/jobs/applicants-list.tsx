...
export function ApplicantsList({ jobId, jobTitle, open, onOpenChange }: ApplicantsListProps) {
  ...
  useEffect(() => {
    ...
    const fetchApplicants = async () => {
      setLoading(true);
      try {
        const { data } = await api.post<{ data: Applicant[]; total: number; page: number; totalPages: number }>(
          API_ENDPOINTS.JOB_APPLICANTS_LIST,
          { jobId, page, limit }
        );
                  ...
                  {applicants.map((a) => (
                    <TableRow key={a._id}>
                      <TableCell className="font-medium">
                        ...
                        <div className="text-xs text-muted-foreground sm:hidden mt-1">{a.email}</div>
                      </TableCell>
